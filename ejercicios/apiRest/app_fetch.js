//Creando un CRUD usando la api de fetch
const d = document,
...
const getAllFruits = () => {
    fetch("http://localhost:8000/frutas")
        .then((respuesta) => {
            if (respuesta.ok) {
        ...
        .catch((err) => {
            let mensajeError = err.statusText || "A ocurrido un error";
...
d.addEventListener("submit", (ev) => {
    ...
    if (!ev.target.id.value) {
        ...
        fetch("http://localhost:8000/frutas", opciones)
            .then((respuesta) => {
                if (respuesta.ok) {
                    return respuesta.json();
                } else {
        ...
        fetch(`http://localhost:8000/frutas/${ev.target.id.value}`, opciones)
            .then((respuesta) => {
                if (respuesta.ok) {
                    return respuesta.json();
                } else {
...
d.addEventListener("click", (ev) => {
    ...
    if (ev.target.matches(".btn-eliminar")) {
        ...
        if (isDelete) {
            ...
            fetch(`http://localhost:8000/frutas/${ev.target.dataset.id}`, opciones)
                .then((respuesta) => {
                    if (respuesta.ok) {
                        return respuesta.json();
                    } else {
